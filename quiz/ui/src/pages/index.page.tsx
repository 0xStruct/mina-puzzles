...
export default function Home() {
  const [state, setState] = useState({
    ...
    hasBeenSetup: false,
    accountExists: false,
    solutionHash: null as null | Field,
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!state.hasBeenSetup) {
        ...
        const solutionHash = await zkappWorkerClient.getSolutionHash();
        console.log(`Current state in zkApp: ${solutionHash.toString()}`);
        setDisplayText("");
  ...
  const onRefreshState = async () => {
    ...
    const solutionHash = await state.zkappWorkerClient!.getSolutionHash();
    setState({ ...state, solutionHash });
    console.log(`Current state in zkApp: ${solutionHash.toString()}`);
    setDisplayText("");
  };
      ...
      <>
        ...
        {/*<div style={{ justifyContent: "center", alignItems: "center" }}>
          <div className={styles.center} style={{ padding: 0 }}>
            Current state in zkApp: {state.solutionHash!.toString()}{" "}
          </div>
          <button
            className={styles.card}
