  ...
  methods: {
    ...
    move: {
      ...
      method: (
        ...
        const player1Id = previousProof.publicOutput.player1Id;
        const player2Id = previousProof.publicOutput.player2Id;
        let gameId = previousProof.publicOutput.gameId;

        // let player0Hash = Poseidon.hash([Poseidon.hash([Field(0)]), gameId]);
        ...
        //     for (let j = 0; j < 3; j++) {
        //       let token = "_";
        //       if (board.board[i][j].isSome.toBoolean()) {
        //         token = board.board[i][j].value.toBoolean() ? "x" : "o";
        //       }
